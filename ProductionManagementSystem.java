...
public class ProductionManagementSystem {
    ...
    static class Stock implements StockService {
        ...
        public synchronized boolean use(String product, int qty) {
        ...
        public synchronized void add(String product, int qty) {
            if (stock.containsKey(product)) {
                stock.put(product,
                        stock.get(product) + qty);
            }
    ...
    static class Processor {
        ...
        public String process(Order order) {
            if (stock.use(order.product, order.quantity)) {
                order.status = "COMPLETED";
                return "Order " + order.id + " completed";
            }
            order.status = "FAILED";
            return "Order " + order.id + " failed";
        }
    }
    ...
    static void reflection(Order order) {
        try {
            ...
            Method m = p.getClass().getMethod("process", Order.class);
            System.out.println(m.invoke(p, order));
        } catch (Exception e) {
    ...
    static void createOrder() {
        ...
        orders.add(OrderFactory.create(id, customer, product, qty, price));
        save();
